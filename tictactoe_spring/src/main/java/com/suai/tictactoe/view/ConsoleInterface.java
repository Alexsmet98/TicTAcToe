...
public class ConsoleInterface {
    ...
    public Coordinates getCoordinates (Board board) {
        log.info("Введите координаты, где хотите нарисовать отметку (пример: 2 4)");
        Scanner scanner = new Scanner(System.in);
        String[] coordinates = scanner.nextLine().split(" ");
        while (!board.canSet(Integer.parseInt(coordinates[0]) - 1, Integer.parseInt(coordinates[1]) - 1)){
            log.info("Введите координаты, где не находиться Х или О");
